...
merge m:1 provider_number Year using temp_hrrp, nogenerate

tab month if round(ratio_ami,0.01)==round(ami,0.01) & ratio_ami!=. & ami!=. & Year==2013
tab month if round(ratio_ami,0.01)==round(ami,0.01) & ratio_ami!=. & ami!=. & Year==2014
tab month if round(ratio_ami,0.01)==round(ami,0.01) & ratio_ami!=. & ami!=. & Year==2015
tab month if round(ratio_ami,0.01)==round(ami,0.01) & ratio_ami!=. & ami!=. & Year==2016

foreach x of varlist ami hf pn hk copd {
  gen diff_`x'=abs(ratio_`x'-`x') if ratio_`x'!=. & ratio_`x'!=0 & `x'!=. & `x'!=0
}
gen month_match=((Year==2010 & month==12) | (Year==2011 & month==10) | (Year==2012 & month==10) | (Year==2013 & month==10) ///
   | (Year==2014 & month==10) | (Year==2015 & month==12) | (Year==2016 & month==10) )
