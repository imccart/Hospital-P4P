...
********************************************************************************
tabstat pen_none pen_one pen_both $PRICE_VARS, by(Year) f(%5.3f)
********************************************************************************
...
foreach y of global PRICE_OUTCOMES{
reghdfe `y' penalty ps2 ps3 ps4 ps_pen*  ${MKT}  ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6  if fips_power~=., absorb(provider_number fips) cluster(provider_number)
est store h_`y'
}
...
foreach y of global PRICE_OUTCOMES{
reghdfe `y' pen_one pen_both ps2 ps3 ps4 ps_one* ps_both*  ${MKT}  ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6  if fips_power~=., absorb(provider_number fips) cluster(provider_number)
est store h_`y'
}
esttab h_lnprice, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
est clear
