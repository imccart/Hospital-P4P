...
tabstat amt_penalty if everpen==1, by(Year)
replace amt_penalty=amt_penalty/10000
...
reghdfe `y' net_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
foreach y of global PRICE_OUTCOMES{
reghdfe `y' net_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
...
foreach y of global DIS_OUTCOMES{
reghdfe `y' amt_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6 if (everpen==1 ) , absorb(provider_number) cluster(provider_number)
est store h_`y'
}
foreach y of global PRICE_OUTCOMES{
reghdfe `y' amt_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6 if (everpen==1 ), absorb(provider_number) cluster(provider_number)
est store h_`y'
}
esttab h_ln_mcaid_discharges h_ln_mcare_discharges h_ln_total_discharges h_ln_other_discharges h_pindex h_lnprice  h_lnprice_hcci, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
esttab h_medicaid_share h_medicare_share h_other_share h_public_share, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
...
**** Start by looking at just net_penalty
foreach y of global DIS_OUTCOMES{
reghdfe `y' net_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6  if any_forprofit==0, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
foreach y of global PRICE_OUTCOMES{
reghdfe `y' net_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6  if any_forprofit==0, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
*** Nonprofit
esttab h_ln_mcaid_discharges h_ln_mcare_discharges h_ln_total_discharges h_ln_other_discharges h_pindex h_lnprice  h_lnprice_hcci, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
...
**** Start by looking at just net_penalty
foreach y of global DIS_OUTCOMES{
reghdfe `y' net_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6  if any_forprofit==1, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
foreach y of global PRICE_OUTCOMES{
reghdfe `y' net_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6  if any_forprofit==1, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
*** FORprofit
esttab h_ln_mcaid_discharges h_ln_mcare_discharges h_ln_total_discharges h_ln_other_discharges h_pindex h_lnprice  h_lnprice_hcci, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
...
**** Now Penalty Amount conditional on (everpen==1 & amt_penalty>=0)
foreach y of global DIS_OUTCOMES{
qui: reghdfe `y' amt_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6 if (everpen==1 ) & any_forprofit==1, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
foreach y of global PRICE_OUTCOMES{
qui: reghdfe `y' amt_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6 if (everpen==1)  & any_forprofit==1, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
*** FORprofit
esttab h_ln_mcaid_discharges h_ln_mcare_discharges h_ln_total_discharges h_ln_other_discharges h_pindex h_lnprice  h_lnprice_hcci, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
...
**** Now Penalty Amount conditional on (everpen==1 & amt_penalty>=0)
foreach y of global DIS_OUTCOMES{
qui: reghdfe `y' amt_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6 evY2 evY3 evY4 evY5 evY6 if (everpen==1) & any_forprofit==1, absorb(provider_number) cluster(provider_number)
est sto h_trend_`y'
qui: test evY2 evY3 evY4 evY5 evY6
scalar `y'_penalty=r(p)
}
foreach y of global PRICE_OUTCOMES{
qui: reghdfe `y' amt_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6 evY2 evY3 evY4 evY5 evY6 if (everpen==1 ) & any_forprofit==1, absorb(provider_number) cluster(provider_number)
est sto h_trend_`y'
qui: test evY2 evY3 evY4 evY5 evY6
scalar `y'_penalty=r(p)
}
...
**** Now Penalty Amount conditional on (everpen==1 & amt_penalty>=0)
foreach y of global DIS_OUTCOMES{
qui: reghdfe `y' amt_penalty i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} m_*  y2 y3 y4 y5 y6 if (everpen==1 ) & Year~=2012, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
foreach y of global PRICE_OUTCOMES{
qui: reghdfe `y' amt_penalty  i.fips_power i.fips_size   ${COUNTY_VARS} ${HOSPITAL_VARS} ${PRICE_VARS}  m_*  y2 y3 y4 y5 y6 if (everpen==1 )  & Year~=2012, absorb(provider_number) cluster(provider_number)
est store h_`y'
}
esttab h_ln_mcaid_discharges h_ln_mcare_discharges h_ln_total_discharges h_ln_other_discharges h_pindex h_lnprice  h_lnprice_hcci, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
esttab h_medicaid_share h_medicare_share h_other_share h_public_share, b(%5.3f) se(%5.3f) star(* 0.1 ** 0.05 *** 0.01) 
